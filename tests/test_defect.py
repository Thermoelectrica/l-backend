...
@pytest.mark.asyncio
async def test_create_defect(api_client: AsyncClient, defect_data, defect_id):
    """Test creating a new defect (server_modified_at ignored for new defects)"""
    response = await api_client.put("/defect", json=defect_data)
...
@pytest.mark.asyncio
async def test_create_defect_with_null_server_modified_at(
    api_client: AsyncClient, defect_id, equipment_id
):
...
@pytest.mark.asyncio
async def test_get_defect(api_client: AsyncClient, defect_data, defect_id):
    """Test retrieving a defect using by_id endpoint"""
    await api_client.put("/defect", json=defect_data)

    response = await api_client.get(f"/defect/by_id/{defect_id}")
    assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_get_nonexistent_defect(api_client: AsyncClient):
...
@pytest.mark.asyncio
async def test_get_all_defects(api_client: AsyncClient, defect_data, equipment_id):
    ...
    await api_client.put("/defect", json=defect_data)
    await api_client.put("/defect", json=defect_data_2)

    # Get all defects
    response = await api_client.get("/defect/all")
    assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_get_defects_by_plant_id(api_client: AsyncClient, defect_data, plant_id):
    """Test retrieving defects by plant ID"""
    await api_client.put("/defect", json=defect_data)

    response = await api_client.get(f"/defect/by_plant_id/{plant_id}")
    assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_update_defect_with_correct_timestamp(api_client: AsyncClient, defect_data):
    """Test updating a defect with correct server_modified_at"""
    create_response = await api_client.put("/defect", json=defect_data)
...
@pytest.mark.asyncio
async def test_concurrent_modification_detected(api_client: AsyncClient, defect_data):
...
@pytest.mark.asyncio
async def test_force_mode_ignores_timestamp(api_client: AsyncClient, defect_data):
    ...
    response = await api_client.put("/defect?force=true", json=defect_data)
    assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_defect_with_defect_type_id(api_client: AsyncClient, defect_data):
...
@pytest.mark.asyncio
async def test_is_deleted_honored_for_defect(api_client: AsyncClient, defect_data):
    ...
    get_response = await api_client.get(f"/defect/by_id/{defect_data['id']}")
    assert get_response.status_code == 200
    assert get_response.json()["is_deleted"] is True


@pytest.mark.asyncio
async def test_get_defects_by_plant_with_modified_since_filter(
    api_client: AsyncClient, defect_data, plant_id, equipment_id
):
    ...
    response1 = await api_client.put("/defect", json=defect_data)
    ...
    response2 = await api_client.put("/defect", json=defect_data_2)
    ...
    response = await api_client.get(f"/defect/by_plant_id/{plant_id}?modified_since={timestamp1}")
    assert response.status_code == 200
    ...
    response = await api_client.get(f"/defect/by_plant_id/{plant_id}?modified_since={timestamp2}")
    assert response.status_code == 200
    filtered_defects = response.json()
    filtered_ids = [d["id"] for d in filtered_defects]
    assert str(defect_id_1) not in filtered_ids
...
@pytest.mark.asyncio
async def test_equipment_defects_always_empty(api_client: AsyncClient, defect_data, equipment_id):
    ...
    response = await api_client.get(f"/equipment/by_id/{equipment_id}")
    assert response.status_code == 200
